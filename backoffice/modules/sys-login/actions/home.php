...
if (isset($_POST["submit"])) { print_r($_POST);
	if (filter_var($_POST["email"], FILTER_VALIDATE_EMAIL) && !empty($_POST["password"])) { print 1;
		$user = new user(); $_POST["password"] = $user->getSecurePassword($_POST["password"]);
		...
		if ($source->num_rows > 0) {
			...
			if (setcookie($cfg->system->cookie, ($data["id"].".".$data["password"]), time() + ($cfg->system->cookie_time * 60))) {
